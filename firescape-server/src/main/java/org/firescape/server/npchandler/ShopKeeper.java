...
public class ShopKeeper implements NpcHandler {
  ...
  public void handleNpc( Npc npc, Player player ) throws Exception {
    Shop shop = world.getShop(npc.getLocation());
    if (shop == null) {
      return;
    }
    try {
      ...
      if (shop.getGreeting() != null) {
        ...
        if (shop.getGreeting().equals("Good day monsieur, Would you like ze nice freshly baked bread?")) {
          if (player.npcThief[0] == true) {
            player.informOfNpcMessage(new ChatMessage(npc, org.firescape.server.event.Thieving.StealChats[org
              .firescape.server.event.Thieving.Rands(org.firescape.server.event.Thieving.StealChats.length)], player));
            return;
          }
        }
