                ...
                public void action() {
                  ...
                  if (option == 1) {
                    player.informOfNpcMessage(
                            new ChatMessage(npc, "Ok. Come back when you have sheared those sheep!", player));
                    return;
                  }
                  if (option == 0 && player.getInventory().countId(207) >= 20) {
                    ...
                    player.getActionSender().sendMessage(
                            "@gre@Congratulations! You have just completed the: @or1@Sheep Shearer @gre@quest!");
                    player.getActionSender().sendMessage("@gre@You gained @or1@1 @gre@quest point!");
                  } else
                    player.informOfNpcMessage(
                            new ChatMessage(npc, "It appears you don't have all my wool yarn balls!", player));
                }
              });
            }
          });
                ...
                public void action() {
                  ...
                  if (option == 0) {
                    owner.informOfNpcMessage(
                            new ChatMessage(npc, "I need you to go and find a tool to shear those sheep,", owner));
                    world.getDelayedEventHandler().add(new ShortEvent(owner) {
                      public void action() {
                        owner.informOfNpcMessage(
                                new ChatMessage(npc, "spin it and bring me 20 wool yarn balls. Good luck", owner));
                        world.getDelayedEventHandler().add(new ShortEvent(owner) {
                          public void action() {
                            owner
                                    .informOfChatMessage(new ChatMessage(owner, "Ok, I will go get them right away!", npc));
