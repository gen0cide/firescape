            ...
            DelayedEvent.world.getDelayedEventHandler().add(new ShortEvent(owner) {
              public void action() {
                if (Formulae.cutWeb()) {
                  ...
                  DelayedEvent.world.delayedSpawnObject(object.getLoc(), 15000);
                } else {
        ...
        switch (object.getID()) {
          ...
          case 491: // Range
            if (item.getID() == 622) { // Seaweed (Glass)
              ...
              ItemCookingDef cookingDef = item.getCookingDef();
              if (cookingDef == null) {
                owner.getActionSender().sendMessage("Nothing interesting happens.");
                return;
              }
          ...
          case 813: // Furnace
            if (item.getID() == 172) { // Gold Bar (Crafting)
              DelayedEvent.world.getDelayedEventHandler().add(new MiniEvent(owner) {
                public void action() {
              ...
              ItemSmeltingDef def = smeltingDef;
              DelayedEvent.world.getDelayedEventHandler().add(new ShortEvent(owner) {
                public void action() {
          ...
          case 178: // Potters Oven
            ...
            InvItem result = new InvItem(resultID, 1);
            int exp = xp;
            boolean fail = Formulae.crackPot(reqLvl, owner.getCurStat(12));
            showBubble();
