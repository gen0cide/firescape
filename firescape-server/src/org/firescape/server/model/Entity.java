x`package org.firescape.server.model;
...
public class Entity {
  ...
  private boolean isMapBlocking(Entity e, int x, int y, byte bit) {
    ...
    if ((val & 64) != 0 && (e instanceof Npc || e instanceof Player || (e instanceof Item && !((Item) e).isOn(x, y))
        || (e instanceof GameObject && !((GameObject) e).isOn(x, y)))) { // There
                                                                         // is
                                                                         // an
                                                                         // object
                                                                         // here,
  ...
  public int[] nextStep(int myX, int myY, Entity e) {
    ...
    if (myX > e.getX()) {
      ...
      myXBlocked = isBlocking(e, myX + 1, myY, 2); // Check left tiles right
                                                   // wall
      newX = myX + 1;
    }
    ...
    if (myY > e.getY()) {
      ...
      myYBlocked = isBlocking(e, myX, myY + 1, 1); // Check bottom tiles top
                                                   // wall
      newY = myY + 1;
    }
