...
public class Doric implements NpcHandler {
  ...
  public void handleNpc(final Npc npc, final Player player) throws Exception {
    ...
    if (player.getDoricsQuestStatus() == 1) {
                ...
                public void action() {
                  ...
                  if (option == 0 && player.getDoricsQuestStatus() == 1 && player.getInventory().countId(149) > 5
                    ...
                    player.getActionSender().sendMessage(
                        "@gre@Congratulations! You have just completed the: @or1@Doric's Quest @gre@quest!");
                    player.getActionSender().sendMessage("@gre@You gained @or1@1 @gre@quest point!");
                  } else
                    player.informOfNpcMessage(
                        new ChatMessage(npc, "It doesn't look like you have my materials at the moment!", player));
                }
              });
            }
          });
      ...
      player.informOfNpcMessage(
          new ChatMessage(npc, "Oh, it seems I am short on a few materials. Can you help me?", player));
      player.setBusy(true);
      world.getDelayedEventHandler().add(new ShortEvent(player) {
        public void action() {
          owner.setBusy(false);
                ...
                public void action() {
                  ...
                  if (option == 0) {
                    owner.informOfNpcMessage(
                        new ChatMessage(npc, "Thank you! I am missing the following materials:", owner));
                    world.getDelayedEventHandler().add(new ShortEvent(owner) {
                      public void action() {
                        owner.informOfNpcMessage(new ChatMessage(npc,
                            "6 Clay, 4 Copper Ore, 2 Iron Ore. Return to me when you have found them!", owner));
