...
import org.firescape.server.entityhandling.defs.extras.ItemDartTipDef;
import org.firescape.server.entityhandling.defs.extras.ItemGemDef;
import org.firescape.server.entityhandling.defs.extras.ItemHerbDef;
import org.firescape.server.entityhandling.defs.extras.ItemHerbSecond;
...
public class InvUseOnItem implements PacketHandler {
          ...
          public void handleReply(final int option, final String reply) {
            ...
            if (owner.getCurStat(12) < reqLvl) {
              owner.getActionSender().sendMessage(
                  "You need a crafting level of " + reqLvl + " to make a " + result.getDef().getName() + ".");
              return;
            }
  ...
  private boolean doHerbSecond(Player player, final InvItem second, final InvItem unfinished,
      final ItemHerbSecond def) {
      ...
      public void action() {
        ...
        owner.setMenuHandler(new MenuHandler(options) {
          public void handleReply(final int option, final String reply) {
            ...
            if (owner.getCurStat(12) < reqLvl) {
              owner.getActionSender().sendMessage(
                  "You need a crafting level of " + reqLvl + " to make " + result.getDef().getName() + ".");
              return;
            }
