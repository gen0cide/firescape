  ...
  public void handleNpc(final Npc npc, final Player player) throws Exception {
    if (player.getDruidicRitualStatus() == 3) {
      player.informOfNpcMessage(
          new ChatMessage(npc, "Sanfew informed me of your help. May you be blessed with the art of Herblaw!", player));
            ...
            public void handleReply(final int option, final String reply) {
              ...
              world.getDelayedEventHandler().add(new ShortEvent(player) {
                public void action() {
                  ...
                  if (option == 1) {
                    player
                        .informOfNpcMessage(new ChatMessage(npc, "Practice makes perfect! Don't give it up!", player));
                    return;
                  }
                  if (option == 0) {
                    player.informOfNpcMessage(
                        new ChatMessage(npc, "That's good to hear! My teachings paid off on you!", player));
                ...
                public void action() {
                  ...
                  if (option == 0) {
                    owner.informOfNpcMessage(
                        new ChatMessage(npc, "As it happens, Sanfew is in need of a little help!", owner));
                    world.getDelayedEventHandler().add(new ShortEvent(owner) {
                      public void action() {
                        ...
                        world.getDelayedEventHandler().add(new ShortEvent(owner) {
                          public void action() {
                            owner.informOfNpcMessage(
                                new ChatMessage(npc, "Sanfew will be wandering around there.", owner));
