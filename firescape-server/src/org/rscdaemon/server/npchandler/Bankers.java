              ...
              public void action() {
                if (option == 0) {
                  ...
                  owner.informOfNpcMessage(
                      new ChatMessage(npc, "Certainly " + (owner.isMale() ? "sir" : "miss"), owner));
                  world.getDelayedEventHandler().add(new ShortEvent(owner) {
                    public void action() {
                      owner.setBusy(false);
                  ...
                  world.getDelayedEventHandler().add(new ShortEvent(owner) {
                    public void action() {
                      owner
                          .informOfNpcMessage(new ChatMessage(npc, "This is a branch of the bank of runescape", owner));
                      world.getDelayedEventHandler().add(new ShortEvent(owner) {
                        public void action() {
                        ...
                        public void handleReply(final int option2, final String reply) {
                          ...
                          world.getDelayedEventHandler().add(new ShortEvent(owner) {
                            public void action() {
                              if (option2 == 0) {
                                owner.informOfNpcMessage(
                                    new ChatMessage(npc, "We will look after your items and money for you", owner));
                                world.getDelayedEventHandler().add(new ShortEvent(owner) {
                                  public void action() {
                                    owner.informOfNpcMessage(new ChatMessage(npc,
                                        "So leave your valuables with us if you want to keep them safe", owner));
                                    // npc.unblock();
                                    owner.setBusy(false);
                                  }
                                });
                                ...
                                owner.setBusy(true);
                                owner.informOfNpcMessage(new ChatMessage(npc,
                                    "Yes we did, but people kept on coming into our branches outside of Varrock",
                                    owner));
                                world.getDelayedEventHandler().add(new ShortEvent(owner) {
                                  public void action() {
                                    owner.informOfNpcMessage(
                                        new ChatMessage(npc, "And telling us our signs were wrong", owner));
                                    world.getDelayedEventHandler().add(new ShortEvent(owner) {
                                      public void action() {
                                        owner.informOfNpcMessage(new ChatMessage(npc,
                                            "As if we didn't know what town we were in or something!", owner));
                                        // npc.unblock();
                                        owner.setBusy(false);
                                      }
                                    });
