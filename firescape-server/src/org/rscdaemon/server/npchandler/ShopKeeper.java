        ...
        if (shop.getGreeting().equals("Good day monsieur, Would you like ze nice freshly baked bread?")) {
          if (player.npcThief[0] == true) {
            player.informOfNpcMessage(
                new ChatMessage(npc, org.rscdaemon.server.event.Thieving.StealChats[org.rscdaemon.server.event.Thieving
                    .Rands(org.rscdaemon.server.event.Thieving.StealChats.length)], player));
            return;
          }
        }
