...
public class Kolodion implements NpcHandler {
  ...
  public void handleNpc(final Npc npc, final Player player) throws Exception {
    if (player.getGuthixSpellCast() == 100 && player.getSaradominSpellCast() == 100
        && player.getZamorakSpellCast() == 100) {
      player.informOfNpcMessage(
          new ChatMessage(npc, "Congratulations. You can now cast all god spells outside the arena!", player));
      return;
    }
    if (player.getGuthixSpellCast() > 0 && player.getGuthixSpellCast() < 100
      ...
      player.informOfNpcMessage(
          new ChatMessage(npc, "You still need to cast spells in the arena. Do you want to go there now?", player));
      player.setBusy(true);
    ...
    } else if (player.getGuthixSpellCast() == 0 && player.getSaradominSpellCast() == 0
        && player.getZamorakSpellCast() == 0) {
      player.informOfNpcMessage(
          new ChatMessage(npc, "Cast 100 of each god spell inside the arena to use them outside", player));
      player.setBusy(true);
      world.getDelayedEventHandler().add(new ShortEvent(player) {
        public void action() {
          owner.setBusy(false);
                ...
                public void action() {
                  ...
                  if (option == 0) {
                    owner.informOfNpcMessage(new ChatMessage(npc,
                        "Once you enter the mage arena, your client information will change!", owner));
                    world.getDelayedEventHandler().add(new ShortEvent(owner) {
                      public void action() {
                        owner.informOfNpcMessage(new ChatMessage(npc,
                            "It will change into a casting counter, so you can see your progress!", owner));
                        world.getDelayedEventHandler().add(new ShortEvent(owner) {
                          public void action() {
                            owner.informOfNpcMessage(
                                new ChatMessage(npc, "I will now teleport you closer to the arena. Good luck!", owner));
