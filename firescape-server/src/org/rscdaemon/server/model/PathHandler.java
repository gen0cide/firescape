...
public class PathHandler {
  ...
  /**
   * Updates the point in the path to the next one assuming we are not finished
   */
  ...
  protected int[] getNextCoords(int startX, int destX, int startY, int destY) {
    try {
      ...
      if (startX > destX) {
        ...
        myXBlocked = isBlocking(startX + 1, startY, 2); // Check left tiles
                                                        // right wall
        coords[0] = startX + 1;
      }
      ...
      if (startY > destY) {
        ...
        myYBlocked = isBlocking(startX, startY + 1, 1); // Check bottom tiles
                                                        // top wall
        coords[1] = startY + 1;
      }
