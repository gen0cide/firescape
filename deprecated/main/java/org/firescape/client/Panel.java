...
public class Panel {
  ...
  int[][] controlCrowns;
  String[][] controListEntriesExtra1;
  String[][] controListEntriesExtra2;
  ...
  public Panel(Surface surface, int max) {
    ...
    controlCrowns = new int[max][];
    controListEntriesExtra1 = new String[max][];
    controListEntriesExtra2 = new String[max][];
  ...
  public int rgb2longMod(int i, int j, int k) {
    return Utility.rgb2long((redMod * i) / 114, (greenMod * j) / 114, (blueMod * k) / 176);
  }
    ...
    if (lastmb == 1) {
      for (int i1 = 0; i1 < controlCount; i1++) {
        if (controlShown[i1] &&
            ...
            mouseX >= controlX[i1] &&
            mouseY >= controlY[i1] &&
            mouseX <= controlX[i1] + controlWidth[i1] &&
            mouseY <= controlY[i1] + controlHeight[i1]) {
    ...
    if (lastmb == 1 || mouseMetaButtonHeld > 20) {
      for (int j1 = 0; j1 < controlCount; j1++) {
        if (controlShown[j1] &&
            ...
            mouseX >= controlX[j1] &&
            mouseY >= controlY[j1] &&
            mouseX <= controlX[j1] + controlWidth[j1] &&
            mouseY <= controlY[j1] + controlHeight[j1]) {
  ...
  public void keyPress(int key) {
    ...
    } else if (focusControlIndex != -1 && controlShown[focusControlIndex] && controlType[focusControlIndex] == 10) {
      if (key == 10 || key == 13) {
        controlClicked[focusControlIndex] = true;
      }
    }
  ...
  public void drawPanel() {
    for (int id = 0; id < controlCount; id++) {
      if (controlShown[id]) {
        if (controlType[id] == 0)// text
        {
          drawText(id, controlX[id], controlY[id], controlText[id], controlTextSize[id], 0);
        } else if (controlType[id] == 1)// text (centered)
        {
          drawText(id,
                   controlX[id] - surface.textWidth(controlText[id], controlTextSize[id]) / 2,
                   controlY[id],
                   controlText[id],
                   controlTextSize[id],
                   0
          );
        } else if (controlType[id] == 2)// component gradient bg
        {
          drawBox(controlX[id], controlY[id], controlWidth[id], controlHeight[id]);
        } else if (controlType[id] == 3)// horiz line
        {
          drawLineHoriz(controlX[id], controlY[id], controlWidth[id]);
        } else if (controlType[id] == 4)// text list (no interaction)
        {
          drawTextList(id,
                       controlX[id],
                       controlY[id],
                       ...
                       controlHeight[id],
                       controlTextSize[id],
                       controlListEntries[id],
                       controlListEntryCount[id],
                       controlFlashText[id],
                       controlCrowns[id]
          );
        } else if (controlType[id] == 5 || controlType[id] == 6)// input text
        {
          drawTextInput(id,
                        controlX[id],
                        controlY[id],
        ...
        } else if (controlType[id] == 7)// option list horiz
        {
          drawOptionListHoriz(id, controlX[id], controlY[id], controlTextSize[id], controlListEntries[id]);
        } else if (controlType[id] == 8)// option list vert
        {
          drawOptionListVert(id, controlX[id], controlY[id], controlTextSize[id], controlListEntries[id]);
        } else if (controlType[id] == 9)// text list (interaction)
        {
          drawTextListInteractive(id,
                                  controlX[id],
                                  controlY[id],
                                  controlWidth[id],
                                  controlHeight[id],
        ...
        } else if (controlType[id] == 11)// rounded box
        {
          drawRoundedBox(controlX[id], controlY[id], controlWidth[id], controlHeight[id]);
        } else if (controlType[id] == 12)// image
        {
          drawPicture(controlX[id], controlY[id], controlTextSize[id]);
        } else if (controlType[id] == 14)// checkbox
        {
          drawCheckbox(id, controlX[id], controlY[id], controlWidth[id], controlHeight[id]);
        }
      }
    }
  ...
  protected void drawText(int control, int x, int y, String text, int textSize, int crown) {
    int y2 = y + surface.textHeight(textSize) / 3;
    drawstring(control, x, y2, text, textSize, crown);
  }
  ...
  protected void drawLineHoriz(int x, int y, int width) {
    surface.drawLineHoriz(x, y, width, 0xffffff); // TODO this is 0 in 233
  }

  protected void drawTextList(
    int control, int x, int y, int width, int height, int textSize, String listEntries[], int listEntryCount, int flashText, int[] crowns
  ) {
    int displayedEntryCount = height / surface.textHeight(textSize);
    if (flashText > listEntryCount - displayedEntryCount) {
      flashText = listEntryCount - displayedEntryCount;
    }
    if (flashText < 0) {
      flashText = 0;
    }
    controlFlashText[control] = flashText;
    if (displayedEntryCount < listEntryCount) {
      ...
      int j3 = ((height - 27 - cornerBottomLeft) * flashText) / (listEntryCount - displayedEntryCount);
      if (mouseButtonDown == 1 && mouseX >= cornerTopRight && mouseX <= cornerTopRight + 12) {
        if (mouseY > y && mouseY < y + 12 && flashText > 0) {
          flashText--;
        }
        if (mouseY > (y + height) - 12 && mouseY < y + height && flashText < listEntryCount - displayedEntryCount) {
          flashText++;
        }
        controlFlashText[control] = flashText;
      }
      if (mouseButtonDown == 1 &&
          (mouseX >= cornerTopRight && mouseX <= cornerTopRight + 12 ||
           mouseX >= cornerTopRight - 12 &&
           mouseX <= cornerTopRight + 24 &&
        ...
        if (mouseY > y + 12 && mouseY < (y + height) - 12) {
          ...
          flashText = (l3 * listEntryCount) / (height - 24);
          if (flashText > listEntryCount - displayedEntryCount) {
            flashText = listEntryCount - displayedEntryCount;
          }
          if (flashText < 0) {
      ...
      j3 = ((height - 27 - cornerBottomLeft) * flashText) / (listEntryCount - displayedEntryCount);
    ...
    for (int entry = flashText; entry < listEntryCount; entry++) {
      drawstring(control, x + 2, y2, listEntries[entry], textSize, crowns != null ? crowns[entry] : 0);
  ...
  protected void drawTextInput(int control, int x, int y, int width, int height, String text, int textSize) {
    ...
    int y2 = y + surface.textHeight(textSize) / 3;
    drawstring(control, x, y2, text, textSize, 0);
  }
  ...
  protected void drawOptionListHoriz(int control, int x, int y, int textSize, String listEntries[]) {
    ...
    for (int idx = 0; idx < listEntryCount; idx++) {
      ...
      if (mouseX >= left &&
          mouseX <= left + surface.textWidth(listEntries[idx], textSize) &&
          mouseY <= bottom &&
          mouseY > bottom - surface.textHeight(textSize)) {
        if (controlUseAlternativeColour[control]) {
          colour = 0x808080;
        } else {
          colour = 0xffffff;
  ...
  protected void drawOptionListVert(int control, int x, int y, int textSize, String listEntries[]) {
    ...
    for (int idx = 0; idx < listEntryCount; idx++) {
      ...
      if (mouseX >= x - entryTextWidth / 2 &&
          mouseX <= x + entryTextWidth / 2 &&
          mouseY - 2 <= listTotalTextHeightMid &&
          mouseY - 2 > listTotalTextHeightMid - surface.textHeight(textSize)) {
        if (controlUseAlternativeColour[control]) {
          colour = 0x808080;
        } else {
          colour = 0xffffff;
  ...
  protected void drawTextListInteractive(
    int control, int x, int y, int width, int height, int textSize, String listEntries[], int listEntryCount, int l1
  ) {
    int displayedEntryCount = height / surface.textHeight(textSize);
    if (displayedEntryCount < listEntryCount) {
      ...
      if (mouseButtonDown == 1 &&
          (mouseX >= right && mouseX <= right + 12 ||
           mouseX >= right - 12 && mouseX <= right + 24 && controlListScrollbarHandleDragged[control])) {
        if (mouseY > y + 12 && mouseY < (y + height) - 12) {
          controlListScrollbarHandleDragged[control] = true;
    ...
    for (int k3 = l1; k3 < listEntryCount; k3++) {
      ...
      if (mouseX >= x + 2 &&
          mouseX <= x + 2 + surface.textWidth(listEntries[k3], textSize) &&
          mouseY - 2 <= i3 &&
          mouseY - 2 > i3 - surface.textHeight(textSize)) {
        if (controlUseAlternativeColour[control]) {
          i4 = 0x808080;
        } else {
          i4 = 0xffffff;
  ...
  protected void drawstring(int control, int x, int y, String text, int textSize, int crown) {
    int color;
    if (controlUseAlternativeColour[control]) {
      color = 0xffffff;
    } else {
    ...
    surface.drawstring(text, x, y, textSize, color, crown);
  }
  ...
  protected void drawTextList(
    int control, int x, int y, int width, int height, int textSize, String listEntries[], int listEntryCount, int flashText
  ) {
    drawTextList(control, x, y, width, height, textSize, listEntries, listEntryCount, flashText, null);
  }
  ...
  public int addTextList(int x, int y, int width, int height, int size, int maxLength, boolean altCol) {
    ...
    controlWidth[controlCount] = width;
    controlHeight[controlCount] = height;
    controlUseAlternativeColour[controlCount] = altCol;
    controlTextSize[controlCount] = size;
    ...
    controlCrowns[controlCount] = new int[maxLength];
    controListEntriesExtra1[controlCount] = new String[maxLength];
    controListEntriesExtra2[controlCount] = new String[maxLength];
    return controlCount++;
  }

  public int addTextListInput(
    int x, int y, int width, int height, int size, int maxLength, boolean maskText, boolean altCol
  ) {
    controlType[controlCount] = 5;
    controlShown[controlCount] = true;
    controlMaskText[controlCount] = maskText;
    controlClicked[controlCount] = false;
    controlTextSize[controlCount] = size;
    controlUseAlternativeColour[controlCount] = altCol;
    controlX[controlCount] = x;
  ...
  public int addTextInput(
    int x, int y, int width, int height, int size, int maxLength, boolean flag, boolean flag1
  ) {
  ...
  public int addTextListInteractive(int x, int y, int width, int height, int textSize, int maxLength, boolean altCol) {
  ...
  public int addOptionListHoriz(
    int x, int y, int textSize, int maxListCount, boolean useAltColour
  ) {
  ...
  public void addListEntry(int control, int index, String text) {
    addListEntry(control, index, text, null, null, 0);
  }

  public void addListEntry(int control, int index, String text, String extra1, String extra2, int crown) {
    controlListEntries[control][index] = text;
    controlCrowns[control][index] = crown;
    controListEntriesExtra1[control][index] = extra1;
    controListEntriesExtra2[control][index] = extra2;
    if (index + 1 > controlListEntryCount[control]) {
      controlListEntryCount[control] = index + 1;
    }
  }

  public void removeListEntry(int control, String text, boolean flash) {
    removeListEntry(control, text, flash, null, null, 0);
  }

  public void removeListEntry(int control, String text, boolean flash, String extra1, String extra2, int crown) {
    int id = controlListEntryCount[control]++;
    if (id >= controlInputMaxLen[control]) {
      id--;
      controlListEntryCount[control]--;
      for (int k = 0; k < id; k++) {
        controlListEntries[control][k] = controlListEntries[control][k + 1];
        controlCrowns[control][k] = controlCrowns[control][k + 1];
        controListEntriesExtra1[control][k] = controListEntriesExtra1[control][k + 1];
        controListEntriesExtra2[control][k] = controListEntriesExtra2[control][k + 1];
      }

    }
    controlListEntries[control][id] = text;
    controlCrowns[control][id] = crown;
    controListEntriesExtra1[control][id] = text;
    controListEntriesExtra2[control][id] = text;
    if (flash) {
  ...
  public int isMouseButtonDown(int control) {
    return controlListEntryMouseButtonDown[control];
  }

  public String getListEntryExtra1(int control, int entry) {
    return controListEntriesExtra1[control][entry];
  }

  public String getListEntryExtra2(int control, int entry) {
    return controListEntriesExtra2[control][entry];
  }
}
