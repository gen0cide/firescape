...
import org.firescape.spriteeditor.data.DataFileDecrypter;
import org.firescape.spriteeditor.data.DataOperations;
import org.firescape.spriteeditor.entityhandling.EntityHandler;
...
public class SpriteLoader {
   ...
   *
   * @param draw
   *   the sprite drawer to render to
   */
  ...
  private void loadMedia() {
    ...
    loadAnimation(MEDIA_START + 9,
                  "media",
                  DataOperations.loadDataFile("bubble.dat", 0, media),
                  mediaIndex,
                  1,
                  mediaMap
    );
    loadAnimation(MEDIA_START + 10,
                  "media",
                  DataOperations.loadDataFile("runescape.dat", 0, media),
                  mediaIndex,
                  1,
                  mediaMap
    );
    loadAnimation(MEDIA_START + 11,
                  "media",
                  DataOperations.loadDataFile("splat.dat", 0, media),
                  mediaIndex,
                  3,
                  mediaMap
    );
    loadAnimation(MEDIA_START + 14,
                  "media",
                  DataOperations.loadDataFile("icon.dat", 0, media),
                  mediaIndex,
                  8,
                  mediaMap
    );
    loadAnimation(MEDIA_START + 22,
                  "media",
                  DataOperations.loadDataFile("hbar.dat", 0, media),
                  mediaIndex,
                  1,
                  mediaMap
    );
    loadAnimation(MEDIA_START + 23,
                  "media",
                  DataOperations.loadDataFile("hbar2.dat", 0, media),
                  mediaIndex,
                  1,
                  mediaMap
    );
    loadAnimation(MEDIA_START + 24,
                  "media",
                  DataOperations.loadDataFile("compass.dat", 0, media),
                  mediaIndex,
                  1,
                  mediaMap
    );
    loadAnimation(MEDIA_START + 25,
                  "media",
                  DataOperations.loadDataFile("buttons.dat", 0, media),
                  mediaIndex,
                  2,
                  mediaMap
    );

    loadAnimation(SCROLLBAR_START,
                  "media",
                  DataOperations.loadDataFile("scrollbar.dat", 0, media),
                  mediaIndex,
                  2,
                  mediaMap
    );
    loadAnimation(SCROLLBAR_START + 2,
                  "media",
                  DataOperations.loadDataFile("corners.dat", 0, media),
                  mediaIndex,
                  4,
                  mediaMap
    );
    loadAnimation(SCROLLBAR_START + 6,
                  "media",
                  DataOperations.loadDataFile("arrows.dat", 0, media),
                  mediaIndex,
                  2,
                  mediaMap
    );

    loadAnimation(PROJECTILE_START,
                  "media",
                  DataOperations.loadDataFile("projectile.dat", 0, media),
                  mediaIndex,
                  7,
                  mediaMap
    );
    ...
    for (int pack = 1; spritesLeft > 0; pack++) {
      ...
      spritesLeft -= 30;
      loadAnimation(ITEM_START + (pack - 1) * 30,
                    "media.object",
                    DataOperations.loadDataFile("objects" + pack + ".dat", 0, media),
                    mediaIndex,
                    frameCount > 30 ? 30 : frameCount,
                    mediaMap
      );
    }
  ...
  private final void loadTextures() {
    ...
    for (int tileIndex = 0; tileIndex < 55; tileIndex++) {
      ...
      drawer.reset();
      loadAnimation(TEXTURE_TEMP,
                    "texture",
                    DataOperations.loadDataFile(s + ".dat", 0, textures),
                    textureIndex,
                    1,
                    textureMap
      );
      ...
      if (animationName != null && animationName.length() > 0) {
        ...
        loadAnimation(TEXTURE_TEMP,
                      "texture",
                      DataOperations.loadDataFile(animationName + ".dat", 0, textures),
                      textureIndex,
                      1,
                      textureMap
        );
  ...
  private void loadAnimation(int frameStart, String packName, byte[] dataImage, byte[] dataInfo, int frameCount, TreeMap<Integer, Sprite> map) {
    ...
    for (int c = 0; c < colours.length - 1; c++) {
      colours[c + 1] = ((dataInfo[infoPointer] & 0xff) << 16) +
                       ((dataInfo[infoPointer + 1] & 0xff) << 8) +
                       (dataInfo[infoPointer + 2] & 0xff);
      infoPointer += 3;
    }
