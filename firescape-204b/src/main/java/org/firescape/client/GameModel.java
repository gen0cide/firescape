...
public class GameModel {
  ...
  public void merge(GameModel pieces[], int count, boolean transState) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      for (int srcF = 0; srcF < source.numFaces; srcF++) {
        ...
        int dstF = createFace(source.faceNumVertices[srcF],
                              dstVs,
  ...
  public void relight() {
    ...
    for (int face = 0; face < numFaces; face++) {
      ...
      int normZ;
      for (normZ = bX * cY - cX * bY; normX > 8192 ||
                                      normY > 8192 ||
                                      normZ > 8192 ||
                                      normX < -8192 ||
                                      normY < -8192 ||
  ...
  public void light() {
    ...
    for (int face = 0; face < numFaces; face++) {
      if (faceIntensity[face] != magic) {
        faceIntensity[face] = (faceNormalX[face] * lightDirectionX +
                               faceNormalY[face] * lightDirectionY +
                               faceNormalZ[face] * lightDirectionZ) / divisor;
      }
    }
    ...
    for (int v = 0; v < numVertices; v++) {
      if (normalMagnitude[v] > 0) {
        vertexIntensity[v] = (normalX[v] * lightDirectionX +
                              normalY[v] * lightDirectionY +
                              normalZ[v] * lightDirectionZ) / (divisor * normalMagnitude[v]);
      }
    }
  ...
  public void project(int cameraX, int cameraY, int cameraZ, int cameraPitch, int cameraRoll, int cameraYaw, int viewDist, int clipNear) {
    apply();
    if (z1 > Scene.frustumNearZ ||
        z2 < Scene.furstumFarZ ||
        x1 > Scene.frustumMinX ||
        x2 < Scene.frustumMaxX ||
        y1 > Scene.furstumMinY ||
        y2 < Scene.furstumMaxY) {
      visible = false;
      return;
    }
    visible = true;
