...
public class Surface implements ImageProducer, ImageObserver {
  ...
  static {
    characterIds = new int[256];
    for (int chr = 0; chr < 256; chr++) {
      int index = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789!\"£$%^&*()-_=+[{]};:'@#~,<.>/?\\| ".indexOf(
        chr);
      if (index == -1) {
        index = 74;// '='
      }
      characterIds[chr] = index * 9;
    }
  ...
  public void setCrownStartId(int id) {
    crownStartId = id;
  }
  ...
  public synchronized void addConsumer(ImageConsumer imageconsumer) {
    ...
    imageconsumer.setHints(ImageConsumer.TOPDOWNLEFTRIGHT | ImageConsumer.COMPLETESCANLINES | ImageConsumer.SINGLEPASS);
  }
  ...
  public void drawGradient(int x, int y, int width, int height, int colourTop, int colourBottom) {
    ...
    for (int k3 = 0; k3 < height; k3 += vertInc) {
      if (k3 + y >= boundsTopY && k3 + y < boundsBottomY) {
        int newColour = ((btmRed * k3 + topRed * (height - k3)) / height << 16) +
                        ((btmGreen * k3 + topGreen * (height - k3)) / height << 8) +
                        (btmBlue * k3 + topBlue * (height - k3)) / height;
        for (int i4 = -width; i4 < 0; i4++) {
          pixels[pixelIdx++] = newColour;
        }
  ...
  public void parseSprite(int spriteId, byte spriteData[], byte indexData[], int frameCount) {
    ...
    for (int i = 0; i < colourCount - 1; i++) {
      colours[i + 1] = ((indexData[indexOff] & 0xff) << 16) +
    ...
    for (int id = spriteId; id < spriteId + frameCount; id++) {
      ...
      spritePixels[id] = null;
      spriteTranslate[id] = spriteTranslateX[id] != 0 || spriteTranslateY[id] != 0;
      if (unknown == 0) {
  ...
  public void readSleepWord(int spriteId, byte spriteData[]) {
    int pixels[] = spritePixels[spriteId] = new int[10200];
  ...
  public void drawWorld(int spriteId) {
    ...
    int spritePixels[] = this.spritePixels[spriteId];
  ...
  public void drawSpriteMinimap(int sprite, int x, int y, int width, int height) {// used from World
    ...
    int pixel = 0;
    spritePixels[sprite] = new int[area];
    for (int xx = x; xx < x + width; xx++) {
      for (int yy = y; yy < y + height; yy++) {
        spritePixels[sprite][pixel++] = pixels[xx + yy * width2];
      }
    }
  }
  ...
  public void drawSpriteAlpha(int x, int y, int spriteId, int alpha) {
    ...
    if (spritePixels[spriteId] == null) {
      drawSpriteAlpha(pixels,
                      spriteColoursUsed[spriteId],
                      spriteColourList[spriteId],
                      j1,
                      size,
                      width,
                      height,
      ...
      drawSpriteAlpha(pixels, spritePixels[spriteId], 0, j1, size, width, height, extraXSpace, j2, yInc, alpha);
      return;
    }
  }

  private void drawSpriteAlpha(
    int dest[], byte coloursUsed[], int colourList[], int listPos, int size, int width, int height, int extraXSpace, int j1, int yInc, int alpha
  ) {
    int i2 = 256 - alpha;
    for (int j2 = -height; j2 < 0; j2 += yInc) {
      for (int k2 = -width; k2 < 0; k2++) {
        ...
        if (l2 != 0) {
          ...
          int i3 = dest[size];
          dest[size++] = ((l2 & 0xff00ff) * alpha + (i3 & 0xff00ff) * i2 & 0xff00ff00) +
                         ((l2 & 0xff00) * alpha + (i3 & 0xff00) * i2 & 0xff0000) >> 8;
        } else {
          size++;
        }
      }
  ...
  private void drawSpriteAlpha(
    int dest[], int src[], int i, int srcPos, int size, int width, int height, int extraXSpace, int k1, int yInc, int alpha
  ) {
    int j2 = 256 - alpha;
    for (int k2 = -height; k2 < 0; k2 += yInc) {
      for (int l2 = -width; l2 < 0; l2++) {
        ...
        if (i != 0) {
          int i3 = dest[size];
          dest[size++] = ((i & 0xff00ff) * alpha + (i3 & 0xff00ff) * j2 & 0xff00ff00) +
                         ((i & 0xff00) * alpha + (i3 & 0xff00) * j2 & 0xff0000) >> 8;
        } else {
          size++;
        }
      }
  ...
  private void transparentScale(
    int dest[], int src[], int i, int j, int k, int destPos, int i1, int j1, int k1, int l1, int i2, int j2, int yInc, int alpha
  ) {
    int i3 = 256 - alpha;
    try {
      int j3 = j;
      for (int k3 = -k1; k3 < 0; k3 += yInc) {
        ...
        for (int i4 = -j1; i4 < 0; i4++) {
          ...
          if (i != 0) {
            int j4 = dest[destPos];
            dest[destPos++] = ((i & 0xff00ff) * alpha + (j4 & 0xff00ff) * i3 & 0xff00ff00) +
                              ((i & 0xff00) * alpha + (j4 & 0xff00) * i3 & 0xff0000) >> 8;
          } else {
            destPos++;
          }
          j += l1;
  ...
  public void spriteClipping(int x, int y, int width, int height, int spriteId, int colour) {
    try {
      ...
      plotScale(pixels, spritePixels[spriteId], 0, i2, j2, j3, l3, width, height, k2, l2, k1, yInc, colour);
      return;
    } catch (Exception Ex) {
      System.out.println("error in sprite clipping routine");
    }
  ...
  private void plotScale(
    int target[], int pixels[], int i, int j, int k, int l, int i1, int width, int height, int l1, int i2, int j2, int yInc, int colour
  ) {
  ...
  private void drawMinimap(
    int ai[], int ai1[], int i, int j, int k, int l, int i1, int j1, int k1, int l1
  ) {
    for (i = k1; i < 0; i++) {
  ...
  private void drawMinimapTranslate(
    int ai[], int ai1[], int i, int j, int k, int l, int i1, int j1, int k1, int l1
  ) {
  ...
  public void spriteClipping(int x, int y, int width, int height, int spriteId) {
    try {
      ...
      plotScale(pixels, spritePixels[spriteId], 0, l1, i2, i3, k3, width, height, j2, k2, spriteWidth, yInc);
      return;
    } catch (Exception Ex) {
      System.out.println("error in sprite clipping routine");
    }
  ...
  private void plotScale(
    int dest[], int src[], int i, int j, int k, int destPos, int i1, int j1, int k1, int l1, int i2, int j2, int k2
  ) {
    try {
      int l2 = j;
  ...
  public void spriteClipping(
    int x, int y, int w, int h, int sprite, int colour1, int colour2, int l1, boolean flag
  ) {
    try {
      if (colour1 == 0) {
        colour1 = 0xffffff;
      ...
      if (colour2 == 0xffffff) {
        if (spritePixels[sprite] != null) {
          if (!flag) {
            transparentSpritePlot(pixels,
                                  spritePixels[sprite],
                                  0,
                                  k2,
                                  l2,
        ...
        if (!flag) {
          transparentSpritePlot(pixels,
                                spriteColoursUsed[sprite],
                                spriteColourList[sprite],
                                0,
                                k2,
                                l2,
                                j4,
  ...
  private void transparentSpritePlot(
    int dest[], int src[], int i, int j, int k, int destPos, int i1, int j1, int k1, int l1, int i2, int j2, int k2, int l2, int i3
  ) {
    int i4 = j2 >> 16 & 0xff;
    int j4 = j2 >> 8 & 0xff;
    int k4 = j2 & 0xff;
  ...
  private void transparentSpritePlot(
    int dest[], byte colourIdx[], int colours[], int i, int j, int k, int l, int i1, int j1, int k1, int l1, int i2, int j2, int k2, int l2, int i3
  ) {
    int i4 = j2 >> 16 & 0xff;
    int j4 = j2 >> 8 & 0xff;
    int k4 = j2 & 0xff;
  ...
  private void transparentSpritePlot(
    int dest[], int src[], int i, int j, int k, int destPos, int i1, int j1, int k1, int l1, int i2, int j2, int k2, int l2, int i3, int j3
  ) {
    int j4 = j2 >> 16 & 0xff;
    int k4 = j2 >> 8 & 0xff;
    int l4 = j2 & 0xff;
  ...
  private void transparentSpritePlot(
    int dest[], byte coloursUsed[], int colourList[], int i, int j, int k, int l, int i1, int j1, int k1, int l1, int i2, int j2, int k2, int l2, int i3, int j3
  ) {
    int j4 = j2 >> 16 & 0xff;
    int k4 = j2 >> 8 & 0xff;
    int l4 = j2 & 0xff;
  ...
  public void drawstring(String text, int x, int y, int font, int colour) {
    drawstring(text, x, y, font, colour, 0);
  }
  ...
  public int textWidth(String text, int fontId) {
    ...
    for (int idx = 0; idx < text.length(); idx++) {
      if (text.charAt(idx) == '@' && idx + 4 < text.length() && text.charAt(idx + 4) == '@') {
        idx += 4;
      } else if (text.charAt(idx) == '@' && text.substring(idx + 1, idx + 3).equals("0x")) {
        if (idx + 9 < text.length() && text.charAt(idx + 9) == '@') {
          idx += 9;
        } else if (idx + 11 < text.length() && text.charAt(idx + 11) == '@') {
          idx += 11;
        }
      } else if (text.charAt(idx) == '~' && idx + 4 < text.length() && text.charAt(idx + 4) == '~') {
        idx += 4;
      } else {
        total += font[characterIds[text.charAt(idx)] + 7];
      }
    }
  ...
  public void drawstring(String text, int x, int y, int font, int colour, int crown) {
    try {
      if (Version.CLIENT > 204 && crown > 0) {
        int crownSprite = crownStartId + (crown - 1);
        if (spriteColoursUsed[crownSprite] != null) {
          drawSprite(x, y, spriteHeight[crownSprite]); // TODO wut
          x += spriteWidth[crownSprite] + 5;
        }
      }
      ...
      for (int idx = 0; idx < text.length(); idx++) {
        if (text.charAt(idx) == '@' &&
            (idx + 4 < text.length() && text.charAt(idx + 4) == '@' ||
             (text.charAt(idx + 1) == '0' &&
              text.charAt(idx + 2) == 'x' &&
              (idx + 9 < text.length() && text.charAt(idx + 9) == '@' ||
               idx + 11 < text.length() && text.charAt(idx + 11) == '@')))) {
          if (text.substring(idx + 1, idx + 4).equalsIgnoreCase("red")) {
            ...
            colour = 0x40ff00;
          } else if (text.substring(idx + 1).startsWith("0x")) {
            if (text.charAt(idx + 9) == '@') {
              colour = Integer.parseInt(text.substring(idx + 3, idx + 9), 16);
              idx += 5;
            } else {
              alpha = Integer.parseInt(text.substring(idx + 3, idx + 5), 16);
              colour = Integer.parseInt(text.substring(idx + 5, idx + 11), 16);
              idx += 7;
            }
          }
          idx += 4;
          ...
          if (chr < 0 || chr > characterIds.length) {
            chr = ' ';
          }

          int chrId = characterIds[chr];
          if (alpha == 255) {
            if (loggedIn && colour != 0) {
              drawCharacter(chrId, x + 1, y, 0, fontData);
              drawCharacter(chrId, x, y + 1, 0, fontData);
            }
            drawCharacter(chrId, x, y, colour, fontData);
          } else {
            if (loggedIn && colour != 0) {
              drawCharacterAlpha(chrId, x + 1, y, 0, alpha, fontData);
              drawCharacterAlpha(chrId, x, y + 1, 0, alpha, fontData);
            }
            drawCharacterAlpha(chrId, x, y, colour, alpha, fontData);
          }
          x += fontData[chrId + 7];
        }
      }
  ...
  private void drawCharacter(int chrId, int x, int y, int colour, byte font[]) {
    ...
    int k1 = font[chrId + 3];
    int l1 = font[chrId + 4];
    int i2 = font[chrId] * 0x4000 + font[chrId + 1] * 128 + font[chrId + 2];
    int j2 = i1 + j1 * width2;
    int k2 = width2 - k1;
    int l2 = 0;
    if (j1 < boundsTopY) {
  ...
  private void drawCharacterAlpha(int chrId, int x, int y, int colour, int alpha, byte font[]) {
    int i1 = x + font[chrId + 5];
    int j1 = y - font[chrId + 6];
    int k1 = font[chrId + 3];
    int l1 = font[chrId + 4];
  ...
  private void plotLetter(
    int pixels[], byte font[], int colour, int j, int k, int l, int i1, int j1, int k1
  ) {
    try {
      int l1 = -(l >> 2);
      l = -(l & 3);
  ...
  private void plotLetterAlpha(
    int pixels[], byte font[], int colour, int alpha, int j, int k, int l, int i1, int j1, int k1
  ) {
    try {
      int bgAlpha = 256 - alpha;
      int red = (colour >> 16 & 0xff) * alpha;
      ...
      for (int i2 = -i1; i2 < 0; i2++) {
        for (int j2 = l1; j2 < 0; j2++) {
          if (font[j++] != 0) {
            int bgRed = (pixels[k] >> 16 & 0xff) * bgAlpha;
            int bgGreen = (pixels[k] >> 8 & 0xff) * bgAlpha;
            int bgBlue = (pixels[k] & 0xff) * bgAlpha;
            pixels[k++] = ((red + bgRed >> 8) << 16) + ((green + bgGreen >> 8) << 8) + (blue + bgBlue >> 8);
          } else {
            k++;
          }
          if (font[j++] != 0) {
    ...
    try {
      ...
      for (int index = 0; index < text.length(); index++) {
        if (text.charAt(index) == '@' && index + 4 < text.length() && text.charAt(index + 4) == '@') {
          index += 4;
        } else if (text.charAt(index) == '@' && text.substring(index + 1, index + 3).equals("0x")) {
          if (index + 9 < text.length() && text.charAt(index + 9) == '@') {
            index += 9;
          } else if (index + 11 < text.length() && text.charAt(index + 11) == '@') {
            index += 11;
          }
        } else if (text.charAt(index) == '~' && index + 4 < text.length() && text.charAt(index + 4) == '~') {
          index += 4;
        } else {
          width += fontdata[characterIds[text.charAt(index)] + 7];
        }
