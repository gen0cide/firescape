...
public class Scene {
  ...
  public int visiblePolygonsCount;
  public Polygon visiblePolygons[];
  ...
  public Scene(Surface surface, int modelLimit, int polygonLimit, int spriteLimit) {
    rampCount = 50;
  ...
  private static void textureScanline(
    int ai[], int ai1[], int i, int j, int k, int l, int i1, int j1, int k1, int l1, int i2, int j2, int k2, int l2
  ) {
    if (i2 <= 0) {
      return;
    }
  ...
  private static void textureTranslucentScanline(
    int ai[], int ai1[], int i, int j, int k, int l, int i1, int j1, int k1, int l1, int i2, int j2, int k2, int l2
  ) {
    if (i2 <= 0) {
      return;
  ...
  private static void textureBackTranslucentScanline(
    int ai[], int i, int j, int k, int ai1[], int l, int i1, int j1, int k1, int l1, int i2, int j2, int k2, int l2, int i3
  ) {
    if (j2 <= 0) {
      return;
    }
  ...
  private static void textureScanline2(
    int ai[], int ai1[], int i, int j, int k, int l, int i1, int j1, int k1, int l1, int i2, int j2, int k2, int l2
  ) {
    if (i2 <= 0) {
      return;
  ...
  private static void textureTranslucentScanline2(
    int ai[], int ai1[], int i, int j, int k, int l, int i1, int j1, int k1, int l1, int i2, int j2, int k2, int l2
  ) {
    if (i2 <= 0) {
      return;
  ...
  private static void textureBackTranslucentScanline2(
    int ai[], int i, int j, int k, int ai1[], int l, int i1, int j1, int k1, int l1, int i2, int j2, int k2, int l2, int i3
  ) {
    if (j2 <= 0) {
      return;
  ...
  public void polygonsIntersectSort(int step, Polygon polygons[], int count) {
    ...
    do {
      ...
      for (int k1 = j1; k1 >= i1 + 1; k1--) {
        ...
        if (polygon.minPlaneX < other.maxPlaneX &&
  ...
  public void render() {
    ...
    for (int model = 0; model < visiblePolygonsCount; model++) {
      ...
      GameModel polygonModel = polygon.model;
      int polyface = polygon.face;
      if (polygonModel == view) {
        int faceverts[] = polygonModel.faceVertices[polyface];
        int face_0 = faceverts[0];
        int vx = polygonModel.vertexViewX[face_0];
        int vy = polygonModel.vertexViewY[face_0];
        int vz = polygonModel.projectVertexZ[face_0];
        int w = (spriteWidth[polyface] << viewDistance) / vz;
        int h = (spriteHeight[polyface] << viewDistance) / vz;
        //int i11 = vy - polygonModel.vertexViewY[faceverts[1]]; // not used
        //int tx = ((polygonModel.vertexViewX[faceverts[1]] - vx) * i11) / h; // redundant
        int tx = polygonModel.vertexViewX[faceverts[1]] - vx;
        int x = vx - w / 2;
        int y = (baseY + vy) - h;
        surface.spriteClipping(x + baseX, y, w, h, spriteId[polyface], tx, (256 << viewDistance) / vz);
        if (mousePickingActive && mousePickedCount < mousePickedMax) {
          x += (spriteTranslateX[polyface] << viewDistance) / vz;
          if (mouseY >= y &&
              mouseY <= y + h &&
              mouseX >= x &&
              mouseX <= x + w &&
              !polygonModel.unpickable &&
              polygonModel.isLocalPlayer[polyface] == 0) {
            mousePickedModels[mousePickedCount] = polygonModel;
            mousePickedFaces[mousePickedCount] = polyface;
            mousePickedCount++;
          }
        }
      } else {
        int plane = 0;
        int light = 0;
        int num_faces = polygonModel.faceNumVertices[polyface];
        int face_verts[] = polygonModel.faceVertices[polyface];
        if (polygonModel.faceIntensity[polyface] != World.colourTransparent) {
        ...
        for (int face = 0; face < num_faces; face++) {
          ...
          if (polygonModel.faceIntensity[polyface] == World.colourTransparent) {
            if (polygon.visibility < 0) {
              ...
              light = polygonModel.lightAmbience +
                      polygonModel.vertexIntensity[vert] +
                      polygonModel.vertexAmbience[vert];
            }
          }
          if (polygonModel.projectVertexZ[vert] >= clipNear) {
            ...
            int vert_end;
            if (face == 0) {
              vert_end = face_verts[num_faces - 1];
            } else {
              vert_end = face_verts[face - 1];
            }
            if (polygonModel.projectVertexZ[vert_end] >= clipNear) {
              int k7 = polygonModel.projectVertexZ[vert] - polygonModel.projectVertexZ[vert_end];
              int i5 = polygonModel.projectVertexX[vert] -
                       ((polygonModel.projectVertexX[vert] - polygonModel.projectVertexX[vert_end]) *
                        (polygonModel.projectVertexZ[vert] - clipNear)) / k7;
              int j6 = polygonModel.projectVertexY[vert] -
                       ((polygonModel.projectVertexY[vert] - polygonModel.projectVertexY[vert_end]) *
                        (polygonModel.projectVertexZ[vert] - clipNear)) / k7;
            ...
            if (face == num_faces - 1) {
              vert_end = face_verts[0];
            } else {
              vert_end = face_verts[face + 1];
            }
            if (polygonModel.projectVertexZ[vert_end] >= clipNear) {
              int l7 = polygonModel.projectVertexZ[vert] - polygonModel.projectVertexZ[vert_end];
        ...
        if (maxY > minY) {
          rasterize(0, 0, num_faces, vertexX, vertexY, vertexZ, polygon.facefill, polygonModel);
        }
      }
    }
  ...
  private void generateScanlines(
    int i, int j, int k, int l, int i1, int ai[], int ai1[], int ai2[], GameModel gameModel, int pid
  ) {
    if (i1 == 3) {
      int k1 = ai1[0] + baseY;
      int k2 = ai1[1] + baseY;
    ...
    if (mousePickingActive && mousePickedCount < mousePickedMax && mouseY >= minY && mouseY < maxY) {
      ...
      if (mouseX >= scanline_1.startX >> 8 &&
          mouseX <= scanline_1.endX >> 8 &&
  ...
  private void rasterize(
    int i, int j, int k, int ai[], int ai1[], int ai2[], int l, GameModel gameModel
  ) {
    if (l == -2) {
      return;
    }
    if (l >= 0) {
      ...
      if (textureDimension[l] == 1) {
        ...
        for (i = minY; i < maxY; i += byte1) {
          ...
          if (i21 <= 0) {
            ...
            textureBackTranslucentScanline(
              raster,
              0,
              0,
              0,
      ...
      for (i = minY; i < maxY; i += byte2) {
        ...
        if (l21 <= 0) {
          ...
          textureBackTranslucentScanline2(
            raster,
            0,
            0,
            0,
  ...
  private boolean separatePolygon(Polygon polygon, Polygon polygon_1) {
    ...
    for (int k4 = 0; k4 < k; k4++) {
      int i1 = ai[k4];
      int i2 = (k2 - gameModel.projectVertexX[i1]) * j3 +
    ...
    for (int l4 = 0; l4 < l; l4++) {
      int j1 = ai1[l4];
      int j2 = (k2 - gameModel_1.projectVertexX[j1]) * j3 +
  ...
  private boolean heuristicPolygon(Polygon polygon, Polygon polygon_1) {
    ...
    for (int i4 = 0; i4 < k; i4++) {
      int i1 = ai[i4];
      int k1 = (i2 - gameModel.projectVertexX[i1]) * l2 +
    ...
    for (int j4 = 0; j4 < l; j4++) {
      int j1 = ai1[j4];
      int l1 = (i2 - gameModel_1.projectVertexX[j1]) * l2 +
