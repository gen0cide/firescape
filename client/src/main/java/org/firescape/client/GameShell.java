...
public class GameShell extends Applet implements Runnable, MouseListener, MouseMotionListener, KeyListener {
  ...
  protected byte[] readDataFile(String file, String description, int percent) {
    ...
    try {
      ...
      java.io.InputStream inputstream = this.getClass().getResourceAsStream("/org/firescape/client/conf/data/" + file);
      ...
      while (read < archiveSizeCompressed) {
        ...
        showLoadingProgress(percent,
                            "Loading " + description + " - " + (5 + (read * 95) / archiveSizeCompressed) + "%"
        );
      }
  ...
  public synchronized void keyPressed(KeyEvent e) {
    ...
    Manager.run("keyPressed", code);
    handleKeyPress(code, chr);
    mouseActionTimeout = 0;
