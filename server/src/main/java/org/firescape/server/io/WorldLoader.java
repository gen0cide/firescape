...
public class WorldLoader {
  ...
  public void loadWorld(World world) {
    try {
      String prefix = "Landscape";
      String suffix = ".rscd";
      InputStream zio = this.getClass().getResourceAsStream("/org/firescape/server/conf/server/data/Landscape.rscd");
      File tempFile = File.createTempFile(prefix, suffix);
      tempFile.deleteOnExit();
      FileOutputStream out = new FileOutputStream(tempFile);
      copyStream(zio, out);
      out.close();
      tileArchive = new ZipFile(tempFile);
    } catch (Exception e) {
      Logger.error(e);
      e.printStackTrace();
    }
