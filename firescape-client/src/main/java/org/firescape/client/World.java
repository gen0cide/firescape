...
public class World {
  ...
  public void method404(int x, int y, int k, int l) {
    ...
    for (int xx = x; xx <= x + k; xx++) {
      for (int yy = y; yy <= y + l; yy++) {
        if ((getObjectAdjacency(xx, yy) & 0x63) != 0 ||
            (getObjectAdjacency(xx - 1, yy) & 0x59) != 0 ||
            (getObjectAdjacency(xx, yy - 1) & 0x56) != 0 ||
            (getObjectAdjacency(xx - 1, yy - 1) & 0x6c) != 0) {
          method425(xx, yy, 35);
        } else {
          method425(xx, yy, 0);
        }
  ...
  public int route(int startX, int startY, int endX1, int endY1, int endX2, int endY2, int routeX[], int routeY[], boolean objects) {
    ...
    while (readPtr != writePtr) {
      ...
      if (x > 0 &&
          y > 0 &&
          (objectAdjacency[x][y - 1] & 0x74) == 0 &&
          (objectAdjacency[x - 1][y] & 0x78) == 0 &&
          (objectAdjacency[x - 1][y - 1] & 0x7c) == 0 &&
          routeVia[x - 1][y - 1] == 0) {
        routeX[writePtr] = x - 1;
        routeY[writePtr] = y - 1;
        writePtr = (writePtr + 1) % size;
      ...
      if (x < 95 &&
          y > 0 &&
          (objectAdjacency[x][y - 1] & 0x74) == 0 &&
          (objectAdjacency[x + 1][y] & 0x72) == 0 &&
          (objectAdjacency[x + 1][y - 1] & 0x76) == 0 &&
          routeVia[x + 1][y - 1] == 0) {
        routeX[writePtr] = x + 1;
        routeY[writePtr] = y - 1;
        writePtr = (writePtr + 1) % size;
      ...
      if (x > 0 &&
          y < 95 &&
          (objectAdjacency[x][y + 1] & 0x71) == 0 &&
          (objectAdjacency[x - 1][y] & 0x78) == 0 &&
          (objectAdjacency[x - 1][y + 1] & 0x79) == 0 &&
          routeVia[x - 1][y + 1] == 0) {
        routeX[writePtr] = x - 1;
        routeY[writePtr] = y + 1;
        writePtr = (writePtr + 1) % size;
      ...
      if (x < 95 &&
          ...
          (objectAdjacency[x][y + 1] & 0x71) == 0 &&
          (objectAdjacency[x + 1][y] & 0x72) == 0 &&
          (objectAdjacency[x + 1][y + 1] & 0x73) == 0 &&
          routeVia[x + 1][y + 1] == 0) {
        routeX[writePtr] = x + 1;
        routeY[writePtr] = y + 1;
        writePtr = (writePtr + 1) % size;
    ...
    if (flag) {
      ...
      for (int j2 = 0; j2 < regionWidth; j2++) {
        for (int i3 = 0; i3 < regionHeight; i3++) {
          ...
          if (getTileDecoration(j2 - 1, i3, plane) > 0 &&
              GameData.tileType[getTileDecoration(j2 - 1, i3, plane) - 1] == 4) {
            i4 = 0;
          }
          if (getTileDecoration(j2, i3 - 1, plane) > 0 &&
              GameData.tileType[getTileDecoration(j2, i3 - 1, plane) - 1] == 4) {
            i4 = 0;
          }
          if (getTileDecoration(j2 - 1, i3 - 1, plane) > 0 &&
              GameData.tileType[getTileDecoration(j2 - 1, i3 - 1, plane) - 1] == 4) {
            i4 = 0;
          }
      ...
      for (int lx = 0; lx < 95; lx++) {
        for (int ly = 0; ly < 95; ly++) {
          ...
          if (getTileDecoration(lx, ly, plane) > 0) {
            ...
            if (decoration_tile_type == 5) {
              if (getWallDiagonal(lx, ly) > 0 && getWallDiagonal(lx, ly) < 24000) {
                if (getTileDecoration(lx - 1, ly, plane, colour_2) != colourTransparent &&
                    getTileDecoration(lx, ly - 1, plane, colour_2) != colourTransparent) {
                  colour = getTileDecoration(lx - 1, ly, plane, colour_2);
                  l14 = 0;
                } else if (getTileDecoration(lx + 1, ly, plane, colour_2) != colourTransparent &&
                           getTileDecoration(lx, ly + 1, plane, colour_2) != colourTransparent) {
          ...
          int i17 = ((getTerrainHeight(lx + 1, ly + 1) - getTerrainHeight(lx + 1, ly)) + getTerrainHeight(lx, ly + 1)) -
                    getTerrainHeight(lx, ly);
          if (colour != colour_1 || i17 != 0) {
            if (l14 == 0) {
              if (colour != colourTransparent) {
                ...
                vert[1] = ly + lx * 96;
                vert[2] = ly + lx * 96 + 1;
                int l21 = gameModel.createFace(3, vert, colourTransparent, colour);
                localX[l21] = lx;
                localY[l21] = ly;
              ...
              if (colour_1 != colourTransparent) {
                ...
                int vert[] = new int[3];
                vert[0] = ly + lx * 96 + 1;
                vert[1] = ly + lx * 96 + 96 + 1;
                vert[2] = ly + lx * 96 + 96;
                int i22 = gameModel.createFace(3, vert, colourTransparent, colour_1);
                localX[i22] = lx;
                localY[i22] = ly;
              ...
              if (colour != colourTransparent) {
                // two color (diagonal) tile SE corner
                int vert[] = new int[3];
                vert[0] = ly + lx * 96 + 1;
                vert[1] = ly + lx * 96 + 96 + 1;
                vert[2] = ly + lx * 96;
                int j22 = gameModel.createFace(3, vert, colourTransparent, colour);
                localX[j22] = lx;
                localY[j22] = ly;
                gameModel.faceTag[j22] = 0x30d40 + j22;
              }
              if (colour_1 != colourTransparent) {
                // two color (diagonal) tile NW corner
                int vert[] = new int[3];
                vert[0] = ly + lx * 96 + 96;
                vert[1] = ly + lx * 96;
                vert[2] = ly + lx * 96 + 96 + 1;
                int k22 = gameModel.createFace(3, vert, colourTransparent, colour_1);
                localX[k22] = lx;
                localY[k22] = ly;
            ...
            vert[1] = ly + lx * 96;
            vert[2] = ly + lx * 96 + 1;
            vert[3] = ly + lx * 96 + 96 + 1;
            int l19 = gameModel.createFace(4, vert, colourTransparent, colour);
            localX[l19] = lx;
            localY[l19] = ly;
      ...
      // ground decorations like floors and bridges
      for (int k4 = 1; k4 < 95; k4++) {
        for (int i6 = 1; i6 < 95; i6++) {
          if (getTileDecoration(k4, i6, plane) > 0 && GameData.tileType[getTileDecoration(k4, i6, plane) - 1] == 4) {
          ...
          } else if (getTileDecoration(k4, i6, plane) == 0 ||
                     GameData.tileType[getTileDecoration(k4, i6, plane) - 1] != 3) {

            if (getTileDecoration(k4, i6 + 1, plane) > 0 &&
                GameData.tileType[getTileDecoration(k4, i6 + 1, plane) - 1] == 4) {
            ...
            if (getTileDecoration(k4 + 1, i6, plane) > 0 &&
                GameData.tileType[getTileDecoration(k4 + 1, i6, plane) - 1] == 4) {
    ...
    for (int i7 = 1; i7 < 95; i7++) {
      for (int k9 = 1; k9 < 95; k9++) {
        ...
        if (roof_nvs > 0) {
          ...
          } else if (getWallDiagonal(i7, k9) > 12000 &&
                     getWallDiagonal(i7, k9) < 24000 &&
                     getWallRoof(i7 + 1, k9 + 1) == 0) {
            int ai9[] = new int[3];
          ...
          } else if (getWallDiagonal(i7, k9) > 0 &&
                     getWallDiagonal(i7, k9) < 12000 &&
                     getWallRoof(i7 + 1, k9 - 1) == 0) {
            int ai10[] = new int[3];
  ...
  public boolean method427(int i, int j) {
    return getWallRoof(i, j) > 0 ||
           getWallRoof(i - 1, j) > 0 ||
           getWallRoof(i - 1, j - 1) > 0 ||
           getWallRoof(i, j - 1) > 0;
  }
