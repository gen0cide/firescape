...
import org.firescape.client.model.Sector;
...
public class EngineHandle {
  ...
  public EngineHandle(Camera camera, GameImage gameImage) {
    ...
    for (int k = 0; k < 64; k++) {
      groundTextureArray[k + 128] = Camera.method305(192 - (int) ((double) k * 1.5D), 144 - (int) ((double) k * 1.5D),
              0);
    }
    for (int l = 0; l < 64; l++) {
  ...
  public int getStepCount(int walkSectionX, int walkSectionY, int x1, int y1, int x2, int y2, int[] walkSectionXArray,
    ...
    while (j2 != i2) {
      ...
      if (k2 > 0 && l2 > 0 && (walkableValue[k2][l2 - 1] & 0x74) == 0 && (walkableValue[k2 - 1][l2] & 0x78) == 0
              && (walkableValue[k2 - 1][l2 - 1] & 0x7c) == 0 && tmpTiles[k2 - 1][l2 - 1] == 0) {
        walkSectionXArray[i2] = k2 - 1;
        walkSectionYArray[i2] = l2 - 1;
        i2 = (i2 + 1) % i3;
      ...
      if (k2 < 95 && l2 > 0 && (walkableValue[k2][l2 - 1] & 0x74) == 0 && (walkableValue[k2 + 1][l2] & 0x72) == 0
              && (walkableValue[k2 + 1][l2 - 1] & 0x76) == 0 && tmpTiles[k2 + 1][l2 - 1] == 0) {
        walkSectionXArray[i2] = k2 + 1;
        walkSectionYArray[i2] = l2 - 1;
        i2 = (i2 + 1) % i3;
      ...
      if (k2 > 0 && l2 < 95 && (walkableValue[k2][l2 + 1] & 0x71) == 0 && (walkableValue[k2 - 1][l2] & 0x78) == 0
              && (walkableValue[k2 - 1][l2 + 1] & 0x79) == 0 && tmpTiles[k2 - 1][l2 + 1] == 0) {
        walkSectionXArray[i2] = k2 - 1;
        walkSectionYArray[i2] = l2 + 1;
        i2 = (i2 + 1) % i3;
      ...
      if (k2 < 95 && l2 < 95 && (walkableValue[k2][l2 + 1] & 0x71) == 0 && (walkableValue[k2 + 1][l2] & 0x72) == 0
              && (walkableValue[k2 + 1][l2 + 1] & 0x73) == 0 && tmpTiles[k2 + 1][l2 + 1] == 0) {
        walkSectionXArray[i2] = k2 + 1;
        walkSectionYArray[i2] = l2 + 1;
        i2 = (i2 + 1) % i3;
  ...
  public void method407(int i, int j, int k, int l) {
    ...
    for (int i1 = i; i1 <= i + k; i1++) {
      for (int j1 = j; j1 <= j + l; j1++) {
        if ((getWalkableValue(i1, j1) & 0x63) != 0 || (getWalkableValue(i1 - 1, j1) & 0x59) != 0
                || (getWalkableValue(i1, j1 - 1) & 0x56) != 0 || (getWalkableValue(i1 - 1, j1 - 1) & 0x6c) != 0) {
          method419(i1, j1, 35);
        } else {
          method419(i1, j1, 0);
        }
  ...
  public boolean method416(int i, int j) {
    return getRoofTexture(i, j) > 0 || getRoofTexture(i - 1, j) > 0 || getRoofTexture(i - 1, j - 1) > 0
  ...
  public void method421(Model model, int i, int j, int k, int l, int i1) {
    ...
    int i4 = model.method181(4, new int[]{
            i3,
            j3,
            k3,
            l3
