...
public class Menu {
  ...
  public int convertRGBToLongWithModifier(int red, int green, int blue) {
    return GameImage.convertRGBToLong((redModifier * red) / 114, (greeModifier * green) / 114,
            (blueModifier * blue) / 176);
  }
  ...
  public void updateActions(int x, int y, int lastMouseDownButton, int mouseDownButton) {
    ...
    if (lastMouseDownButton == 1) {
      for (int menuObject = 0; menuObject < menuObjectCount; menuObject++) {
        if (menuObjectCanAcceptActions[menuObject] && menuObjectType[menuObject] == 10
                && mouseX >= menuObjectX[menuObject] && mouseY >= menuObjectY[menuObject]
                && mouseX <= menuObjectX[menuObject] + menuObjectWidth[menuObject]
                && mouseY <= menuObjectY[menuObject] + menuObjectHeight[menuObject])
          menuObjectHasAction[menuObject] = true; // if it's a button and
        // clicked
        if (menuObjectCanAcceptActions[menuObject] && menuObjectType[menuObject] == 14
                && mouseX >= menuObjectX[menuObject] && mouseY >= menuObjectY[menuObject]
                && mouseX <= menuObjectX[menuObject] + menuObjectWidth[menuObject]
                && mouseY <= menuObjectY[menuObject] + menuObjectHeight[menuObject])
          anIntArray189[menuObject] = 1 - anIntArray189[menuObject]; // no idea
        // what
        // this is,
        // there is
        // no
    ...
    if (lastMouseDownButton == 1 || mouseClicksConsecutive > 20) {
      for (int j1 = 0; j1 < menuObjectCount; j1++)
        if (menuObjectCanAcceptActions[j1] && menuObjectType[j1] == 15 && mouseX >= menuObjectX[j1]
                && mouseY >= menuObjectY[j1] && mouseX <= menuObjectX[j1] + menuObjectWidth[j1]
    ...
    for (int menuObject = 0; menuObject < menuObjectCount; menuObject++)
      if (menuObjectCanAcceptActions[menuObject])
        if (menuObjectType[menuObject] == 0)
          drawTextAddHeight(menuObject, menuObjectX[menuObject], menuObjectY[menuObject], menuObjectText[menuObject],
                  menuObjectTextType[menuObject]);
        else if (menuObjectType[menuObject] == 1)
          drawTextAddHeight(menuObject,
                  menuObjectX[menuObject]
                          - gameImage.textWidth(menuObjectText[menuObject], menuObjectTextType[menuObject]) / 2,
                  menuObjectY[menuObject], menuObjectText[menuObject], menuObjectTextType[menuObject]);
        else if (menuObjectType[menuObject] == 2)
          method146(menuObjectX[menuObject], menuObjectY[menuObject], menuObjectWidth[menuObject],
          ...
          method150(menuObject, menuObjectX[menuObject], menuObjectY[menuObject], menuObjectWidth[menuObject],
                  menuObjectHeight[menuObject], menuObjectTextType[menuObject], menuListText[menuObject],
                  menuListTextCount[menuObject], anIntArray187[menuObject]);
        else if (menuObjectType[menuObject] == 5 || menuObjectType[menuObject] == 6)
          method145(menuObject, menuObjectX[menuObject], menuObjectY[menuObject], menuObjectWidth[menuObject],
                  menuObjectHeight[menuObject], menuObjectText[menuObject], menuObjectTextType[menuObject]);
        else if (menuObjectType[menuObject] == 7)
          method152(menuObject, menuObjectX[menuObject], menuObjectY[menuObject], menuObjectTextType[menuObject],
                  menuListText[menuObject]);
        else if (menuObjectType[menuObject] == 8)
          method153(menuObject, menuObjectX[menuObject], menuObjectY[menuObject], menuObjectTextType[menuObject],
                  menuListText[menuObject]);
        else if (menuObjectType[menuObject] == 9)
          method154(menuObject, menuObjectX[menuObject], menuObjectY[menuObject], menuObjectWidth[menuObject],
                  menuObjectHeight[menuObject], menuObjectTextType[menuObject], menuListText[menuObject],
                  menuListTextCount[menuObject], anIntArray187[menuObject]);
        else if (menuObjectType[menuObject] == 11)
          method147(menuObjectX[menuObject], menuObjectY[menuObject], menuObjectWidth[menuObject],
                  menuObjectHeight[menuObject]);
          ...
          method142(menuObject, menuObjectX[menuObject], menuObjectY[menuObject], menuObjectWidth[menuObject],
                  menuObjectHeight[menuObject]);
  ...
  protected void method150(int i, int j, int k, int l, int i1, int j1, String as[], int k1, int l1) {
    ...
    if (i2 < k1) {
      ...
      if (mouseButton == 1
              && (mouseX >= j2 && mouseX <= j2 + 12 || mouseX >= j2 - 12 && mouseX <= j2 + 24 && aBooleanArray184[i])) {
  ...
  protected void method145(int i, int j, int k, int l, int i1, String s, int j1) {
    ...
    if (menuObjectType[i] == 5) {
      ...
      if (lastMouseButton == 1 && mouseX >= j - l / 2 && mouseY >= k - i1 / 2 && mouseX <= j + l / 2
              && mouseY <= k + i1 / 2)
        currentFocusHandle = i;
  ...
  protected void method152(int i, int j, int k, int l, String as[]) {
    ...
    for (int j2 = 0; j2 < j1; j2++) {
      ...
      if (menuObjectColourMask[i])
        ...
        k2 = 0;
      if (mouseX >= l1 && mouseX <= l1 + gameImage.textWidth(as[j2], l) && mouseY <= i2
  ...
  protected void method153(int i, int j, int k, int l, String as[]) {
    ...
    for (int k1 = 0; k1 < i1; k1++) {
      ...
      if (mouseX >= j - i2 / 2 && mouseX <= j + i2 / 2 && mouseY - 2 <= j1
  ...
  protected void method154(int i, int j, int k, int l, int i1, int j1, String as[], int k1, int l1) {
    int i2 = i1 / gameImage.messageFontHeight(j1);
    if (i2 < k1) {
      int j2 = (j + l) - 12;
      int l2 = ((i1 - 27) * i2) / k1;
    ...
    for (int k3 = l1; k3 < k1; k3++) {
      ...
      if (menuObjectColourMask[i])
        ...
        i4 = 0;
      if (mouseX >= j + 2 && mouseX <= j + 2 + gameImage.textWidth(as[k3], j1) && mouseY - 2 <= i3
